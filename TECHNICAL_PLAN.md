...
| **错题/典型题讲解** | 答错后 AI 生成互动讲解页（错因分析+图解步骤+变式巩固），首次生成+缓存，练习页+错题本双入口 | MiniMax + 缓存HTML |
| **课件库** | 可浏览的课件列表（本系统形式呈现），点开即学 | TeachAny 课件（自托管） |
| **课件制作** | 家长端输入知识点/专题，AI生成互动课件HTML | MiniMax（家长端入口） |
...
| **试卷真题库** | 500套PDF真题按题截取（含配图），标注知识点/难度/题型，去重选代表性入题库 | 中考真题+调考+区统考PDF |
| **家长看板** | 每日报告、周趋势、薄弱点提醒、学科开关、AI配置、课件制作 | 本系统 daily_stats |
